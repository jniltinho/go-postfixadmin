<template>
  <q-page class="ad-page">

    <!-- ─── Header ─── -->
    <div class="page-header">
      <div>
        <div class="page-title">DOMAIN ALIASES</div>
        <div class="page-subtitle">MANAGE ALIAS DOMAIN MAPPINGS</div>
      </div>
            ...
            <tr v-for="row in pagedRows" :key="row.alias_domain" class="table-row">
              <td class="table-td td-link">
                <div class="cell-with-icon">
                  <q-icon name="swap_horiz" size="14px" class="row-icon" />
                  {{ row.alias_domain }}
                </div>
              </td>
              <td class="table-td">
                <span class="mono-text">{{ row.target_domain }}</span>
              </td>
              <td class="table-td">
        ...
        <div class="modal-body">
          ...
          <div class="info-card">
            <div class="info-card-title">
              <q-icon name="swap_horiz" size="15px" style="margin-right:6px" />
              ALIAS DOMAIN INFORMATION
            </div>

            <div class="form-row2">
              <div class="form-group">
                <label class="form-label">ALIAS DOMAIN <span class="req">*</span></label>
                <select v-model="addForm.alias_domain" class="form-select-plain">
                ...
                <span class="form-hint">Domain that will be the alias (source)</span>
              </div>
              <div class="form-group">
                <label class="form-label">TARGET DOMAIN <span class="req">*</span></label>
                <select v-model="addForm.target_domain" class="form-select-plain">
                  <option value="" disabled>Select target domain...</option>
                  <option v-for="d in domains" :key="d.domain" :value="d.domain">{{ d.domain }}</option>
                </select>
                <span class="form-hint">Domain that will receive the mail (target)</span>
              </div>
            </div>
      ...
      <div class="modal-card">
        <div class="modal-head">
          <span class="modal-head-title">
            ...
            EDIT DOMAIN ALIAS
            <span class="modal-head-sub">— {{ editForm.alias_domain }}</span>
          </span>
            ...
            <div class="info-card">
              ...
              <div class="form-row2">
                <div class="form-group">
                  ...
                  <input :value="editForm.alias_domain" class="form-input form-input-disabled" disabled />
                  <span class="form-hint">Alias domain cannot be changed after creation</span>
                </div>
                <div class="form-group">
                  <label class="form-label">TARGET DOMAIN <span class="req">*</span></label>
                  <select v-model="editForm.target_domain" class="form-select-plain">
                    <option value="" disabled>Select target domain...</option>
                    <option v-for="d in domains" :key="d.domain" :value="d.domain">{{ d.domain }}</option>
                  </select>
                  <span class="form-hint">Domain that receives the mail</span>
                </div>
              </div>
...
// domains that are not already used as an alias_domain
...
<style scoped>
.ad-page { background: #ebf2fe; padding: 24px 28px 40px; }
...
.modal-body { padding: 20px; overflow-y: auto; flex: 1; display: flex; flex-direction: column; gap: 16px; position: relative; }
