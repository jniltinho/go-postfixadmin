    ...
    <div class="bg-white border-2 border-brand-text w-full max-w-2xl max-h-[90vh] flex flex-col">
      ...
      <div class="bg-brand-primary px-6 py-4 flex items-center justify-between flex-shrink-0" style="border-bottom: 2px solid #1e293b;">
        <h3 class="text-lg font-mono font-black uppercase tracking-tight text-white flex items-center">
          ...
          EDIT TRANSPORT
          <span class="text-white opacity-75 font-normal text-sm ml-2">— {{ form.domain }}</span>
        </h3>
      ...
      <div class="overflow-y-auto flex-1 relative min-h-[200px]">
        <div v-if="loading" class="absolute inset-0 bg-white/80 flex flex-col items-center justify-center z-10 gap-3">
          ...
          <form class="p-6 space-y-5" @submit.prevent="submit">
            <div class="border-2 border-brand-text p-4 space-y-4">
              ...
              <div>
                ...
                <input v-model="form.transport" type="text" required placeholder="smtp:[mail.example.com]:25"
                  class="w-full h-10 px-3 border-2 border-brand-text focus:border-brand-primary focus:outline-none font-medium transition-colors text-sm" />
                <p class="text-[10px] text-gray-400 mt-1">Transport definition, e.g. smtp:[host]:port or relay:[host]</p>
              </div>

              <div class="flex items-center">
                <input type="checkbox" id="tp-edit-active" v-model="form.active" class="w-5 h-5 border-2 border-brand-text cursor-pointer" />
                <label for="tp-edit-active" class="ml-2 text-sm font-bold cursor-pointer">Active</label>
              </div>
            </div>
          </form>
        </template>
...
const props = defineProps<{
  modelValue: boolean
  initialData: TransportEditForm | null
  saving: boolean
  loading: boolean
}>()
...
const form = ref<TransportEditForm>({ id: 0, domain: '', transport: '', active: true })
